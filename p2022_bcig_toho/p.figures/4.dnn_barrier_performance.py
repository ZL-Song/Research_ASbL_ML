...
def ds_load(sys, path, ):
    ...
    ypreddir = f'/users/zilins/scratch/2.proj_toho2lig_acy/3.dwnn/dnn_cpu_mod{tag}/y_pred.npy'
...
fo_log = open('dnn_performance_stat.log', 'w')

for path in ['r1ae', 'r2ae', ]:
    for sys in ['toho_amp', 'toho_cex']:
        fo_log.write('\n{0}.{1}\n\n'.format(sys, path, ))
        ...
        ax.set_aspect(1.)   
        fig.savefig(f'dnn_{sys}.{path}_barrier.png')
        
        print(f'Done: dnn_{sys}.{path}_barrier.png')
        fo_log.write('\n')
fo_log.write('\n')
